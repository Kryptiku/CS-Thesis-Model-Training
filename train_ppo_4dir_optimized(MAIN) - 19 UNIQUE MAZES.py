...
class ProgressBarCallback(BaseCallback):
    def __init__(self, total_timesteps, log_file="training_log (Main) QRNG - 19.csv", seed_csv_path=None):
        super().__init__()
        self.pbar = None
    ...
    def _on_training_start(self):
        ...
        with open(self.log_file, 'w', newline='') as f:
            ...
            writer.writerow(['# NOTE: Train 19 - Each environment uses unique/offset mazes in parallel'])
            writer.writerow([])
...
class TrainingMetricsReport:
    ...
    def __init__(self, log_file, model_name="Model", seed_type="PRNG"):
        self.log_file = log_file
        self.model_name = model_name
        self.seed_type = seed_type
        self.df = None
        self.report_dir = "training_reports"
...
<html>
<head>
    <title>Agent Training Metrics Report</title>
    <style>
...
<body>
    <div class="container">
        <h1>🤖 Agent Training Metrics Report</h1>
        ...
        <div class="footer">
            ...
            <p>This report summarizes the training performance of the RL agent on {'PRNG' if 'PRNG' in self.seed_type else 'QRNG'}-generated mazes.</p>
        </div>
    </div>
</body>
</html>
...
if __name__ == "__main__":
    # ===== CONFIGURATION: SWAP CSV HERE =====
    SEED_CSV_PATH = "qrng_seeds.csv"  # Change to "concatenated_qrng.csv" to switch to QRNG
    ...
    MODEL_NAME = "PPO_20x20_Manhattan (MAIN) QRNG - 19 UNIQUE MAZES"
    ...
    model = PPO("MlpPolicy", env, verbose=0, device=device_target,
                learning_rate=3e-4,
                n_steps=2048,
                batch_size=512, 
                n_epochs=10,
    ...
    progress_callback = ProgressBarCallback(TOTAL_TIMESTEPS, seed_csv_path=SEED_CSV_PATH)
    ...
    mean_reward, std_reward = evaluate_policy(model, eval_env, n_eval_episodes=200, deterministic=True)  # More episodes for consistency check
    ...
    # Determine seed type from log filename for proper labeling
    seed_type = "PRNG" if "PRNG" in progress_callback.log_file else "QRNG"
    ...
    print(f"📁 Open 'training_reports/training_report_{seed_type.lower()}.html' to view the full report")
