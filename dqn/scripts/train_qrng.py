...
class ProgressBarCallback(BaseCallback):
    def __init__(self, total_timesteps, log_file="training_log_qrng.csv", seed_csv_path=None):
        super().__init__()
        self.pbar = None
...
if __name__ == "__main__":
    ...
    DQN_DIR = os.path.dirname(SCRIPT_DIR)              # dqn/
    ...
    MODEL_SAVE_PATH = os.path.join(DQN_DIR, "models", "DQN_20x20_Manhattan-QRNG-trial")
    ...
    def make_env(env_id=0):
        ...
        env = LidarMazeEnv(size=20, seed_type="QRNG", max_steps=500, seed_csv_path=SEED_CSV_PATH, env_id=env_id)
        return Monitor(env)
    ...
    # EVALUATION
    print("\n--- RUNNING EVALUATION ---")
    eval_env = DummyVecEnv([lambda env_id=0: Monitor(LidarMazeEnv(size=20, seed_type="QRNG", max_steps=500, seed_csv_path=SEED_CSV_PATH, env_id=env_id))])
