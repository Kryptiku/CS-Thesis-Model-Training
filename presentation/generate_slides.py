...
def build_slide_7_model_design(prs):
    ...
    add_bullet_list(slide, Inches(0.8), Inches(1.8), Inches(5.8), Inches(1.6), [
    ...
    add_bullet_list(slide, Inches(1.0), Inches(3.7), Inches(5.0), Inches(2.5), [
        'Goal Achieved: +250',
        'Wall Collision: -0.75',
        'Step Penalty: -0.005',
...
def build_slide_9_results(prs):
    ...
    p.font.color.rgb = NAVY
    p.alignment = PP_ALIGN.CENTER
    for s in ['Mann-Whitney U: all p > 0.05', 'PERMANOVA: p = 0.7493', 'ERA Hull: PRNG=0.0131, QRNG=0.0125']:
        p2 = tf.add_paragraph()
        p2.text = s
        p2.font.size = Pt(11)
        p2.font.color.rgb = DARK_GRAY
