"""
Script to create time-series graphs comparing all runs for PRNG and QRNG A2C models.
Creates 4 subplots: PRNG Success Rate, QRNG Success Rate, PRNG Rewards, QRNG Rewards.
Each subplot shows individual runs + average line.
"""
...
# Run directories (relative to MODEL_OUTPUT_DIR/training_logs/)
RUN_DIRS = ["log_1", "log_2", "log_3"]
RUN_LABELS = ["Run 1", "Run 2", "Run 3"]

# Output filename for the plots
OUTPUT_FILENAME = "training_comparison.png"

# Colors for each run (distinct colors)
...
def load_training_data(run_dir, seed_type):
    """
    Load training log CSV for a given run and seed type.

    Args:
        run_dir (Path): Path to the run directory (e.g., outputs/training_logs/log_1/)
        seed_type (str): "prng" or "qrng"

    Returns:
        pd.DataFrame: Training data or None if file not found
    """
    csv_path = run_dir / f"training_log_{seed_type}.csv"
...
def create_comparison_plot():
    """Create 4-subplot comparison plot for PRNG and QRNG across all runs."""
    ...
    # Episode arrays for x-axis (saved per seed type for averaging)
    prng_episodes_list = []
    qrng_episodes_list = []

    # Load and plot data for each run
    for idx, run_name in enumerate(RUN_DIRS):
        run_path = MODEL_OUTPUT_DIR / "training_logs" / run_name
        ...
        # Load PRNG data
        prng_df = load_training_data(run_path, "prng")
        if prng_df is not None:
            ...
            ax_prng_success.plot(
                ...
                color=color, linewidth=LINE_WIDTH_RUNS,
            ...
            prng_episodes_list.append(episodes)

        # Load QRNG data
        qrng_df = load_training_data(run_path, "qrng")
        if qrng_df is not None:
            ...
            qrng_episodes_list.append(episodes)
    ...
    if prng_success_data:
        ...
        ax_prng_success.plot(
            prng_episodes_list[0][:min_len], avg_prng_success,
    ...
    if qrng_success_data:
        ...
        ax_qrng_success.plot(
            qrng_episodes_list[0][:min_len], avg_qrng_success,
            color=AVERAGE_COLOR, linewidth=LINE_WIDTH_AVERAGE,
            label="Average", linestyle="-"
        )
    ...
    if prng_reward_data:
        ...
        ax_prng_reward.plot(
            prng_episodes_list[0][:min_len], avg_prng_reward,
            color=AVERAGE_COLOR, linewidth=LINE_WIDTH_AVERAGE,
            label="Average", linestyle="-"
        )
    ...
    if qrng_reward_data:
        ...
        ax_qrng_reward.plot(
            qrng_episodes_list[0][:min_len], avg_qrng_reward,
            color=AVERAGE_COLOR, linewidth=LINE_WIDTH_AVERAGE,
            label="Average", linestyle="-"
        )
    ...
    ax_prng_success.set_title("A2C PRNG - Success Rate % over Episodes", fontsize=14, fontweight="bold")
    ...
    ax_qrng_success.set_title("A2C QRNG - Success Rate % over Episodes", fontsize=14, fontweight="bold")
    ...
    ax_prng_reward.set_title("A2C PRNG - Rewards over Episodes", fontsize=14, fontweight="bold")
    ...
    ax_qrng_reward.set_title("A2C QRNG - Rewards over Episodes", fontsize=14, fontweight="bold")
...
if __name__ == "__main__":
    print(f"Creating A2C training comparison plots from: {MODEL_OUTPUT_DIR}")
    print(f"Runs: {', '.join(RUN_LABELS)}")
